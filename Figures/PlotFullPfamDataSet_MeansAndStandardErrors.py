...
for SpeciesDatabase in SpeciesDatabases:
    ...
    ExtractionStatement = "SELECT PfamUID,PassedIUPredFilters," + MetricsColumn + " FROM "+SpeciesDatabase+"Genomes_DomainMetrics_Complete WHERE UID"
...
plt.title('Pfam Only - %s'%MetricsLabel)
plt.savefig('PfamOnly_%s'%Metric.replace('-',''))
plt.close()
