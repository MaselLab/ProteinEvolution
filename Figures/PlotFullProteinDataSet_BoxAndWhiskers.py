...
# The MySQL database where the protein metrics are stored with their ages

NCBIDatabase = 'NCBIGenomes_ProteinMetrics_Complete'
EnsemblDatabase = 'Genomes_Multicellular_ProteinMetrics'

HomologyGroupIDColumn = 'HomologyGroupID'
PassedIUPredFiltersColumn = 'PassedIUPredFilters'
AgeOfOldestPfamColumn = 'AgeOfOldestPfam'
UIDColumnName = 'ProteinTableUID'
...
for SpeciesDatabase in SpeciesDatabases:
    ...
    ExtractionStatement = "SELECT " + ','.join([HomologyGroupIDColumn,PassedIUPredFiltersColumn,AgeOfOldestPfamColumn,MetricsColumn]) + " FROM "+SpeciesDatabase 
    if Test == True:
        ExtractionStatement += " WHERE "+UIDColumnName+" < 10000"
    ...
    for result in results:
        if result != None:
            HomologyGroupID,PassedFilters,Age,ExtractedMetric = result
            if ExtractedMetric != None and PassedFilters != int(False):
                ExtractedMetric = float(ExtractedMetric)
                if HomologyGroupID not in HomologyGroupDictionary:
                    HomologyGroupDictionary[HomologyGroupID] = {'Metric':[ExtractedMetric],'Age':Age}
                else:
                    HomologyGroupDictionary[HomologyGroupID]['Metric'].append(ExtractedMetric)
...
for HomologyGroupID in HomologyGroupDictionary:
    
    MeanHomologyGroupMetric = np.mean(HomologyGroupDictionary[HomologyGroupID]['Metric'])
    Age = HomologyGroupDictionary[HomologyGroupID]['Age']
    
    if MeanHomologyGroupMetric == 0:
        MeanHomologyGroupMetric = float(1e-20)
    if Age != None:
...
RScript.write('pdf("%s_Vs_Age_FullGenes.pdf",width=10)\n'%MetricsLabel)
...
if MetricChoice == 'ISD':
    ...
    RScript.write('plot(df$Age,df$Metric,type="n",main=Title,xlab="Age (millions of years)",ylab="%s",cex.lab=1.75,col="dodgerblue",pch=20,cex.axis=1.75,cex.main=1.7,ylim=c(-.0000001,0.03))\n'%MetricsLabel)
...
os.remove(TempDataFrameFilename)
